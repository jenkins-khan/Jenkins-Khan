...
abstract class BaseProfile extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the jenkins_url field.
	 * @var        string
	 */
	...
	/**
	 * Get the [jenkins_url] column value.
	 * 
	 * @return     string
	 */
	...
	/**
	 * Set the value of [jenkins_url] column.
	 * 
	 * @param      string $v new value
	 * @return     Profile The current object (for fluent API support)
	 */
	public function setJenkinsUrl($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->jenkins_url !== $v) {
			$this->jenkins_url = $v;
			$this->modifiedColumns[] = ProfilePeer::JENKINS_URL;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->jenkins_url = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->resetModified();
			...
			return $startcol + 3; // 3 = ProfilePeer::NUM_HYDRATE_COLUMNS.
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(ProfilePeer::SF_GUARD_USER_ID)) {
			$modifiedColumns[':p' . $index++]  = '`SF_GUARD_USER_ID`';
		}
		if ($this->isColumnModified(ProfilePeer::JENKINS_URL)) {
			$modifiedColumns[':p' . $index++]  = '`JENKINS_URL`';
		}
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					...
					case '`SF_GUARD_USER_ID`':
						...
						break;
					case '`JENKINS_URL`':
						$stmt->bindValue($identifier, $this->jenkins_url, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setJenkinsUrl($arr[$keys[2]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ProfilePeer::JENKINS_URL)) $criteria->add(ProfilePeer::JENKINS_URL, $this->jenkins_url);
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setJenkinsUrl($this->getJenkinsUrl());
		if ($makeNew) {
			$copyObj->setNew(true);
	...
	public function getsfGuardUser(PropelPDO $con = null)
	{
		if ($this->asfGuardUser === null && ($this->sf_guard_user_id !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->asfGuardUser->addProfiles($this);
			 */
		}
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseProfile:' . $name))
		{
