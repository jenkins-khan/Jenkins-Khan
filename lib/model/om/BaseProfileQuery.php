 ...
 * @method     ProfileQuery orderByJenkinsUrl($order = Criteria::ASC) Order by the jenkins_url column
 *
 ...
 * @method     ProfileQuery groupByJenkinsUrl() Group by the jenkins_url column
 *
 ...
 * @method     Profile findOneByJenkinsUrl(string $jenkins_url) Return the first Profile filtered by the jenkins_url column
 *
 ...
 * @method     array findByJenkinsUrl(string $jenkins_url) Return Profile objects filtered by the jenkins_url column
 *
...
abstract class BaseProfileQuery extends ModelCriteria
{
	...
	{
		$sql = 'SELECT `ID`, `SF_GUARD_USER_ID`, `JENKINS_URL` FROM `profile` WHERE `ID` = :p0';
		try {
	...
	/**
	 * Filter the query on the jenkins_url column
	 *
	 * Example usage:
	 * <code>
	 * $query->filterByJenkinsUrl('fooValue');   // WHERE jenkins_url = 'fooValue'
	 * $query->filterByJenkinsUrl('%fooValue%'); // WHERE jenkins_url LIKE '%fooValue%'
	 * </code>
	 *
	 * @param     string $jenkinsUrl The value to use as filter.
	 *              Accepts wildcards (* and % trigger a LIKE)
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    ProfileQuery The current query, for fluid interface
	 */
	public function filterByJenkinsUrl($jenkinsUrl = null, $comparison = null)
	{
		if (null === $comparison) {
			if (is_array($jenkinsUrl)) {
				$comparison = Criteria::IN;
			} elseif (preg_match('/[\%\*]/', $jenkinsUrl)) {
				$jenkinsUrl = str_replace('*', '%', $jenkinsUrl);
				$comparison = Criteria::LIKE;
			}
		}
		return $this->addUsingAlias(ProfilePeer::JENKINS_URL, $jenkinsUrl, $comparison);
	}
